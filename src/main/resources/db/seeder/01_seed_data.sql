...
-- =============================================================================
-- CONDITIONAL SEEDING - Only run if database is empty
-- =============================================================================
DO $$
BEGIN
    -- Check if we already have agents or customers (real data, not just admin/user)
    IF EXISTS (SELECT 1 FROM public.agent LIMIT 1)
       OR EXISTS (SELECT 1 FROM public.customer LIMIT 1)
       OR EXISTS (SELECT 1 FROM public."user" WHERE login NOT IN ('admin', 'user') LIMIT 1) THEN
        RAISE NOTICE 'Database already contains data. Skipping seeding.';
        RETURN;
    END IF;

    RAISE NOTICE 'Database is empty. Running seed data...';
